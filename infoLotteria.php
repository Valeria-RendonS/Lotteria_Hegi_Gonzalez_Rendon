...
// Recupero ruolo e crediti utente
$ruolo = "ospite";
$crediti_utente = null;

if (isset($_SESSION['utente_id'])) {
    $stmt_r = $pdo->prepare("SELECT ruolo, crediti FROM utente WHERE id = :id");
    $stmt_r->execute(['id' => $_SESSION['utente_id']]);
    $utente = $stmt_r->fetch(PDO::FETCH_ASSOC);
    $ruolo = $utente['ruolo'] ?? "ospite";
    if ($ruolo !== 'admin') {
        $crediti_utente = $utente['crediti'];
    }
}
?>
...
.costo-box { margin:15px 0; font-weight:600; color:#334155; }

.warning { color:#dc2626; font-weight:600; margin-top:10px; display:none; }
</style>
</head>
...
<div class="container-info">
    ...
    <?php if($ruolo === 'admin'): ?>
        <div class="admin-alert">
            <i class="fa-solid fa-lock"></i> Gli amministratori non possono acquistare biglietti.
        </div>
